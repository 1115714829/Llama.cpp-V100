...
### 1.4 工具与作业习惯（2026-09-24 定，**视情况使用——判断优先，非机械戒律**）

1. **活儿在服务器跑，人不陪跑**：长活丢 tmux 具名会话；**等待用"结束符阻塞"**——一条命令内 `while ! grep -q <END_MARKER> <log>; do sleep 30; done; <收账>`（等待耗在脚本里，命中即返回，零轮询零上下文；配 mcp-ssh `toolCallTimeoutMs` 上调）；短查询秒查；绝不长睡裸轮询。
