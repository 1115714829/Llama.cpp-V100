...
ggml_tensor * llama_kv_cache::get_k(ggml_context * ctx, int32_t il, uint32_t n_kv, const slot_info & sinfo) const {
    ...
    const uint32_t n_kv_b = (uint32_t) llama_kv_bucket_n(n_kv, (int64_t) kv_size);

    return ggml_view_4d(ctx, k,
            hparams.n_embd_head_k(il), hparams.n_head_kv(il), n_kv_b, ns,
...
ggml_tensor * llama_kv_cache::get_v(ggml_context * ctx, int32_t il, uint32_t n_kv, const slot_info & sinfo) const {
    ...
    if (!v_trans) {
        ...
        return ggml_view_4d(ctx, v,
                hparams.n_embd_head_v(il), hparams.n_head_kv(il), n_kv_b, ns,
    ...
    // note: v->nb[1] > v->nb[2]
    return ggml_view_4d(ctx, v,
            n_kv_b, hparams.n_head_kv(il), hparams.n_embd_head_v(il), ns,
...
uint32_t llama_kv_cache_context::get_n_kv_bucketed() const {
    return (uint32_t) llama_kv_bucket_n(n_kv, kv->get_size());
}
