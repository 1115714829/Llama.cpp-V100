...
## 4b. 优先级重排（Round 103，依据 §25.6/§25.8/§25.9 的实测与上游查证）

原计划（P-A/P-B/P-C/P-D）是按**旧的每轮账本**排的，那份账本把 `enqueue_us` 当成「AR 时间」。现在的实测口径是：

| 项 | 旧排序依据 | 现状（实测/查证后） |
|---|---|---|
| P-A 把 AR 压到 18 us | -4.8 ms/轮 | **作废为动机**：R1 主机省 4.8 ms 而轮时零改善；TP3 比 TP2 的 enqueue 多 4.4 ms 却总轮时更少 |
| P-B draft 图复用 | 以为是缺陷 | **作废为动机**：`reuse=0` 是 #28549 的设计取舍（单 active 指针是它自己加的），可回收上限只有 alloc 3.62 + build 0.25 ms/轮 |
| P-B draft 算子融合 | 13.6 ms 是 GPU 前向 | **作废为动机**：13.6 ms ≈ draft 主机时间 14.7 ms，不是 GPU 在算 |
| P-D 长上下文 attention | 256K decode 34 -> 60 t/s | **维持**（独立轴，用户真实场景，未动） |
| P-C mask/KV 分桶 | prefill 每块重建 | **维持**（改善 TTFT/prefill，独立轴） |

**重排后的主攻顺序**：
1. **P-A2 步骤 1（本规格）**：先用 `GGML_META_HOST_TIMING` 把 20.7 ms 拆开。**这是当前唯一在关键路径上的未知量**。
2. 按拆分结果二选一：`dev_us` 占大头 -> 2a 整轮单图；`rebuild_us` 占大头 -> 2b（但与 #28549 设计冲突，须先与上游意图对齐）。
3. P-D / P-C（独立轴，可并行派子代理，因为不碰 decode 稳态的图结构）。
4. draft 的 `alloc_us` 3.62 ms/轮（两份 sched 变体）—— 只作为组合拳的备选，单独不值得做。
