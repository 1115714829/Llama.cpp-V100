...
>
> **⏸ 机器状态（2026-09-21 18:35 用户通知）：用户要关机给设备加一块本地磁盘以提升编译速度，开机后会告知。**
> 结论：**在收到用户通知前，不要对 192.168.50.235 发起任何编译或测量**（运行中的任务会被关机打断）。当前无运行中的链/子代理。
> 开机后要做的事（按价值排序；需先问清挂载点与可用空间）：
> 1. **把模型放本地盘**：现在冷加载 255 s 是无盘 NFS 的 114 MB/s 线速，占一臂墙钟的 85-95%（§4.34）。
>    模型（Qwen3.8-27B-Q8_0 29 GB + DFlash2 draft 1.14 GB + Q2_K_XL 9.8 GB）移到本地盘后冷加载预计降到十几秒 => **每臂墙钟降约 4-5 倍**（本项目最稀缺的资源）。
> 2. **build 目录 + `CCACHE_DIR` 放本地盘**（ccache 只覆盖 C/C++、不覆盖 `.cu`；`ggml-backend*.cpp` 属 C++，会变秒级）。
> 3. **源码树放本地盘**：`/root/llm/test/v100-opt/llama.cpp`（不再每次遍历 NFS 头文件）。
> 4. 完成后重跑一次同源基线确认口径没变（期望 97.88 t/s / `f3edac19...`）。
>
> **下一步（按序）**
