...
struct common_speculative_impl {
    ...
    int64_t t_process_us = 0; // total time spent in the target-feature hook (process) in microseconds.
    size_t  n_process    = 0; // number of process() calls
...
struct common_speculative_impl_draft_dflash : public common_speculative_impl {
    ...
    void build_dflash2_selector_cpu(std::vector<int32_t> & cand, std::vector<float> & unary, std::vector<float> & gate) {
        ...
        // optional timing of the two CPU-bound parts (LLAMA_SPEC_TIMING); zero cost when unset
        ...
        // the lm_head output for every block position lives in one host-side logits buffer
        std::vector<const float *> logits_all((size_t) n_tokens, nullptr);
        for (int32_t i = 0; i < n_tokens; ++i) {
            logits_all[i] = llama_get_logits_ith(ctx_dft, i);
            GGML_ASSERT(logits_all[i] && "DFlash2 CPU selector requires the lm_head output");
        }
        const int64_t t_fetch1 = st_enabled ? ggml_time_us() : 0;
    ...
    void draft(common_speculative_draft_params_vec & dparams) override {
        ...
        // optional per-phase timing for the draft round (LLAMA_SPEC_TIMING); zero cost when unset
        static const bool st_enabled = (getenv("LLAMA_SPEC_TIMING") != nullptr);
        static int64_t st_dec = 0;
        ...
        const int64_t st_t2 = st_enabled ? ggml_time_us() : 0;
...
// optional per-round attribution of the speculative overhead (LLAMA_SPEC_TIMING)
static void common_speculative_timing_report(const common_speculative * spec) {
    static const bool enabled = (getenv("LLAMA_SPEC_TIMING") != nullptr);
    if (!enabled || spec == nullptr || spec->impls.empty()) {
        return;
    }

    static int32_t n_tick = 0;
    static int64_t t_prev_process = 0;
    static int64_t t_prev_draft   = 0;
    static int64_t t_prev_accept  = 0;

    n_tick++;
    if (n_tick % 32 != 0 || n_tick <= 32) {
        t_prev_process = spec->impls[0]->t_process_us;
        t_prev_draft   = spec->impls[0]->t_draft_us;
        t_prev_accept  = spec->impls[0]->t_accept_us;
        return;
    }

    const double k = 1.0/32.0;
    const int64_t p = spec->impls[0]->t_process_us;
    const int64_t d = spec->impls[0]->t_draft_us;
    const int64_t a = spec->impls[0]->t_accept_us;

    LOG_INF("%s: spec overhead per round | process=%.2f draft=%.2f accept=%.2f ms (n_process=%zu)\n",
            __func__, (p - t_prev_process)*k/1e3, (d - t_prev_draft)*k/1e3, (a - t_prev_accept)*k/1e3,
            spec->impls[0]->n_process);
...
bool common_speculative_process(common_speculative * spec, const llama_batch & batch) {
    ...
    common_speculative_timing_report(spec);
...
void common_speculative_draft(common_speculative * spec) {
    ...
    // tick once per draft() call, i.e. once per speculative round (process() is a prefill-only hook
    // and accept() can return early, so neither of those is a reliable tick point)
    common_speculative_timing_report(spec);
}
