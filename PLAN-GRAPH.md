...
flowchart TD
  ...
  subgraph TODO["待办节点"]
    ...
    H5["★★ H5 浅上下文台阶假设 (R167)<br/>0.7952 us/ctx-token 段按 P=1 只有 43.8 GB/s<br/>（深段 0.0701/0.0941 => 370-496 GB/s = roofline）<br/>按字节比例的成本不可能在小 n_kv 上低效 10 倍<br/>=> 更可能是 300-8192 之间**一个约 6 ms/step 的离散台阶**<br/>它正落在我们的生产区间 (harness n_kv 只有几百到一千)<br/>验证: 密集深度阶梯 + f16 对照臂 (R167 已派给子代理)"]:::warn
  end
  N0 -->|实测支撑| ADD
  ...
  Z8 -->|它决定形状是否真在变| N6B
  Z8 -.->|判据: 命中距离| N11
  H5 -.->|若成立则 N1/N13/N8* 都是 KV 字节路线, 与台阶无关| N1
  H5 -.->|台阶是每次 FA 调用的固定成本 => 新抓手| N5
  H5 -->|先手量测| Z
  N1 -->|解锁| K3
  METATAX -->|消灭它则解锁| K4
  N6 -->|消灭| METATAX
