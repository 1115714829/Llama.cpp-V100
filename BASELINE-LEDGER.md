...
- **新头号候选（一个 env）**：**UB=2048 × 桶化叠加**——R295 实测 ub2048 = +27% 预填充（817→1038 t/s）与桶化 +38.4%（机制正交）外推叠加 = **1131→~1435 t/s ⇒ TTFT ~165 s ⇒ 距 BL1 1.08x**。判据：TTFT + 回归判（tg/纯步不回退 >2%，SLOTS/显存包络，mask OOM 复查——R295 曾 SLOTS=3 OOM 于 1.7GB mask）。
