...
### R424 (2026-09-26) 首份 [OP] 实测：q8_0 直读内核在跑，但 FA 43.71 ms/轮（66.8%）比暂存路 25.4 还慢

**路由生效的证据**（先澄清：上一轮我 grep 错了文件，e3-run.log 是脚本日志，服务日志是 `e3.log`）：`e3.log` 40 条 `[SM70LONG] SELECTED` + `[SM70EXEC] #1..#4 dev=0..3 q=2 kv=256 hq=6 hkv=1 **Ktype=8**` ⇒ e3 那次 [OP] 表**确实是 q8_0 直读内核**（env 经 tmux 新 server 继承生效）。

**[OP] 表（e3：256K spec-on、gen 320、reps 1、`GGML_CUDA_DISABLE_GRAPHS=1`）**
| op | calls | total | 占比 | avg |
|---|---|---|---|---|
| FLASH_ATTN_EXT | 128 | 5594.8 ms | **66.8%** | **43.71 ms** |
| MUL_MAT | 4475 | 2449.8 ms | 29.3% | 0.547 ms |
| GLU | 508 | 58.5 ms | 0.7% | 0.115 ms |
| CPY | 3048 | 24.8 ms | 0.3% | 0.008 ms |

**判定：带宽赢了、算力输了。** q8_0 每 256 值行 272 B vs f16 512 B（字节量减半），但 FA 反而 25.4 → 43.71 ms。两个嫌疑（都在内核内）：
1. **q8_0 解码是标量逐字节**（R423 为修对齐改成 8 次字节读 + 移位组装，每 8 值 8 读）—— 指令量约 3-4x 于向量读；
2. **非补偿 PV 走"在线路"**，而旧路是补偿方案的延后 PV（R421 条件编译掉的那段）—— 两者算力特征不同。
**注意**：MUL_MAT 也从 25.2 → 19.1（我的改动不可能影响它）⇒ 两表的时段/AL 不同，**跨表比不安全**；下一步必须**同源 A/B**（同一 e3 脚本、`LLAMA_SM70_Q8_0_INPLACE` 开/关两臂、≥2 rep 报离散）。
...
### R430 (2026-09-26) W1 首个有效数字：q8_0 零拷贝比暂存快 1.27x（但数值门值未验，见末段）

**同源 A/B（同一 e3 口径的单卡 8K、`NO_SPEC=1`、IQ1_S、prompt 6955 / gen 200，每次跑后 pkill 清场）**
| 臂 | env | tg | tpot | TTFT | pp |
|---|---|---|---|---|---|
| 路由关（默认 TILE 路） | — | 37.25 t/s | 26.84 ms | 6.82 s | 1019.8 |
| 路由开 + 暂存 f16 | `LLAMA_SM70_LONG_DECODE=1` | 28.00 t/s | 35.71 ms | 6.81 s | 1021.1 |
| **路由开 + q8_0 零拷贝直读** | `+ LLAMA_SM70_Q8_0_INPLACE=1` | **35.55 t/s** | **28.13 ms** | 6.805 s | 1022.0 |

⇒ **直读比暂存快 1.27x**（tpot 35.71→28.13 ms；tg 28.00→35.55，+27%）——**消掉 f16 暂存税的设计目标首次被实测确认** ✓。预填充**毫发无损**（TTFT/pp 三臂一致）。
**8K 域的解读**：8K 下每层 KV 仅 17.8 MB、64 层 ≈1.4 ms/轮（KV 非瓶颈）⇒ 我方路由的固定开销占主导，故直读臂仍略逊于默认 TILE（28.13 vs 26.84）。**256K 是 KV 主导域**（默认路还要多读一倍字节 + 付暂存税），差距应进一步拉开 —— 待 256K TP4 复测。**本数字只在"同一路由内"口径下有效**（8K 尺子，R429 已裁定）。

**⚠ 立即必做（不许跳）**：该直读臂的 PV 走**非补偿在线 PV**（R421 条件编译开出的配置），**数值正确性至今未验证**（文本连贯 ≠ 逐位一致）。⇒ 下一步在单卡做**同模型同 prompt 的 greedy 门值对比**（路由关 vs 直读开，temperature=0、固定 prompt，比对输出 sha / 逐 token 一致），过不了门则本 1.27x 作废并回到 PV 数值问题。

### R431 (2026-09-26) 门值否决 R430 的 1.27x：直读路数值不一致 + 捕获下段错误

**单卡同模型同 prompt（greedy：`temperature=0, top_k=1`，max_tokens=64）门值对比**
| 臂 | 状态 | 结果 |
|---|---|---|
| 路由关（基准） | OK | `sha=f18343e6c154b6ce`，**225 字符** |
| 直读（图捕获开） | **SIGSEGV rc=139** | 崩在 `n_tokens=42` 的 decode，**无 CUDA 报错**（主机侧崩） |
| 直读（图关 + `BLOCKING=1`） | 跑通但**结果不同** | `sha=47997935f78f480d`，**仅 64 字符** ✗ |

⇒ **两个独立缺陷**：① 直读路数值**与基准不一致**（输出长度都不同）；② 图捕获下**段错误**。
⇒ **按 R430 预设的规矩，1.27x 作废**（不许拿未过门的速度数当成果）。R430 的 A/B 数字保留但标注为**待数值确认**。

**下一轮（定位顺序：先分辨两条缺陷各属哪一层）**
1. **数值不一致**：把直读路与**已逐位验证过的 fp8 路**（R409：49152 元素 `abs_err>2e-2`=0）在同形状下对比；若 fp8 对而 q8_0 错 ⇒ 错在 **q8_0 loader/解码**（虚拟 256 行映射或 scale 应用）；若两者都错 ⇒ 错在 **非补偿在线 PV**（R421 条件编译开出的配置）。**这是二选一的关键实验。**
2. **段错误**：`n_tokens=42`（q=42 > 8 ⇒ 本不该进我方路由）却在 decode 崩 ⇒ 需确认是否 `supported()` 之外还有路径把大 q 送进来，或属草稿链；用 `gdb`/`catchsegv` 拿栈，或在适配层入口加 q 的断言打印。
3. 两条都清掉后，重跑 greedy 门值 ⇒ 过了才恢复 1.27x 的效力。

### R432 (2026-09-26) 门值失败的真相：路由改变贪心输出（非崩溃）；并自曝 R409 数值自检是弱测试

**实验（单卡 8K、长 prompt ≈2000 token ⇒ 解码期 kv ≥ 512，落在路由契约内；三臂同 greedy 请求）**
| 臂 | env | 结果 |
|---|---|---|
| `p_off` | 路由关 | `sha=9a93cdb2b5bad9ad`，244 字符 ✓ |
| `p_stg` | `LLAMA_SM70_LONG_DECODE=1` | 无 `choices`（服务端 500） |
| `p_in` | `+ LLAMA_SM70_Q8_0_INPLACE=1` | 无 `choices`（服务端 500） |

**失败性质（读日志定案）**：`rc=0`、**不是崩溃**；服务端异常 = `{"code":500,"message":"The model produced output that does not match the expected peg-native format"}` ⇒ **我方路由改变了模型输出**（贪心解码下逐 token 分叉，末尾触发 harmony PEG 解析失败）。与 R431 的"64 字符 vs 225 字符"同源。
**另注**：`K->ne[1] >= 512` 门槛已恢复（R419 的放开是我引入的洞，已回退），但**不是本次失败的原因**（长 prompt 下解码节点本来就满足 ≥512）。

**两种可能，必须分辨（不许含糊）**
- **(a) 算术次序差异**：分组核（80 splits + combine + 非补偿在线 PV）的求和次序与 TILE 参考不同 ⇒ 低位不同 ⇒ 贪心放大 ⇒ 文本分叉。数学上无害，但**直接撞击项目验收标准「门值 bcda0092 greedy sha 逐位一致」**（该标准之所以可能，是因为 T1C 引擎当年就是按"逐位一致"写的）。若属 (a) ⇒ 要么把内核做成逐位一致（对齐参考的累加/softmax 次序，代价大），要么**请用户裁定门值标准是否对 FA 内核放宽为容差判据**（这是标准变更，不能我单方面改）。
- **(b) 真 bug**：则修。

**自曝（必须记）**：R409 那次"逐位一致"（`abs_err>2e-2`=0、最坏相对误差=0）**是弱测试** —— 我特意把 K/V 造成 **E4M3 可精确表示的正值格点**，误差由构造保证为零，**证明不了真实分布下的保真度**。该结论不得再被引用为"数值已验证"。

**下一轮（分辨实验，替代弱测试）**
1. 用**随机分布**（真实量级、含负值、含非格点值）的 K/V 做合成测试：同一输入分别跑 (i) 参考路径 (ii) 我方 fp8 路 (iii) 我方 q8_0 直读路，输出**容差并报 max |Δ| 与相对误差**（不再要求逐位）、并**同时**报"逐位相同比例"⇒ 用数据判定 (a)/(b)。
