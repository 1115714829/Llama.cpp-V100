...
- **R318 T2 判决（q8_0 KV 直读 = 显存使能件非性能刀；流量模型臆想被历史数据重锤）**：源码注记（8/23-24 实测）：q4-direct 预填充 **-3.6%**（窄 u16 块读 > dequant 转换成本）、**镜像转换全成本仅 ~0.1s/770s（0.013%）**⇒ "KV 流量减半 ⇒ FA +30-60%" = 流量模型错误（FA 读次数不变、字节减半≠时间减半）。**q8-direct 模式已实现**（`LLAMA_SM70_Q8_DIRECT=1`，Path B 遗产）⟹ 降级为 **T1 显存使能件**（回收 ~470 MB f16 镜像 → score workspace 预算）。**主刀回归 T1 = 79T 形态 @ Q2048**（45.9→60+ TFLOPS 形态优势 = 唯一量级空间；红线边界：批≤2048、卡≤4、显存≤14.5GB/卡、KV≤8bit）。
