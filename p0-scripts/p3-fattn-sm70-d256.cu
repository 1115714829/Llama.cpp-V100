...
// Path A: accept any KV type the stock to_fp16 path can convert (production is q8_0).
static bool sm70_d256_kv_type_ok(const ggml_type type) {
    switch (type) {
        case GGML_TYPE_F32:
        case GGML_TYPE_F16:
        case GGML_TYPE_BF16:
...
// Default ON (user 2026-09-22). LLAMA_SM70_D256=0 rolls back to stock MMA_F16.
static bool sm70_env_enabled() {
...
// PER DEVICE (TP runs several devices concurrently in different threads).
static void sm70_decomp_ws_reserve(int id, size_t bytes) {
    auto & st = FLASH_NAMESPACE::sm70_decomp_state(id);
    if (bytes <= st.ws_bytes) {
        return;
    }
    if (st.ws != nullptr) {
        CUDA_CHECK(cudaFree(st.ws));
    }
    CUDA_CHECK(cudaMalloc(&st.ws, bytes));
    st.ws_bytes = bytes;
}

static void sm70_decomp_dev_init(int id) {
    auto & st = FLASH_NAMESPACE::sm70_decomp_state(id);
    if (st.cublas == nullptr) {
        CUBLAS_CHECK(cublasCreate(&st.cublas));
    }
}
...
bool ggml_cuda_sm70_d256_supported(int cc, const ggml_tensor * dst) {
    if (cc != GGML_CUDA_CC_VOLTA || !sm70_env_enabled()) {
        ...
        if (cc == GGML_CUDA_CC_VOLTA) {
            ...
            sm70_d256_probe(cc != GGML_CUDA_CC_VOLTA ? "REJECT: cc!=volta" : "REJECT: env disabled (=0)", cc, Qp, Kp, Vp, Mp);
        }
        return false;
    }
    ...
    // Path A: any KV type the stock to_fp16 path can convert (q8_0 included).
    // F16 is used in-place; everything else dequants into the f16 extra.
    const bool kv_ok = sm70_d256_kv_type_ok(K->type) && sm70_d256_kv_type_ok(V->type);
    if (!kv_ok) {
...
void ggml_cuda_flash_attn_ext_sm70_d256(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if (t1c_on) {
        ...
        fprintf(stderr, "[T1C] ABI send ptr=%p v0=%g\n", (void *) scratch_max,
                (double) scratch_max[0]);
    ...
    if (sm70_fa_decomp()) {
        ...
        if (nb == 1 && !k_direct && !v_direct
            && (Q->type == GGML_TYPE_F16 || Q->type == GGML_TYPE_F32)
            // Q layout is [d, head, q]: one token block = hq*256 contiguous.
            && Q->nb[1] == (int64_t) ggml_row_size(Q->type, SM70_D256_D) * Q->ne[2]
            // staged KV (quantized) is packed; direct f16 needs canonical layout
            && (K->type != GGML_TYPE_F16
                || (K->nb[1] == 2 * SM70_D256_D && K->nb[2] == K->nb[1] * K->ne[1]))
        ...
        sm70_d256_probe("REJECT: decomp T2 shape/type, Path A fallback",
                        cc, Q, K, V, mask);
        fprintf(stderr,
            "[decomp-guard] nb=%d kd=%d vd=%d qtype=%d qnb1=%lld qrow=%lld "
            "qne1=%lld qne2=%lld knb2=%lld kne1=%lld vnb2=%lld vne1=%lld\n",
            nb, (int) k_direct, (int) v_direct, (int) Q->type,
            (long long) Q->nb[1],
            (long long) ggml_row_size(Q->type, SM70_D256_D),
            (long long) Q->ne[1], (long long) Q->ne[2],
            (long long) K->nb[2], (long long) K->ne[1],
            (long long) V->nb[2], (long long) V->ne[1]);
    }
