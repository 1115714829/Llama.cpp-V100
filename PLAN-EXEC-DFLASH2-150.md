...
### 1.4b E0 结果（零代码，从既有日志读出，19:2x）——**四段里三段已知，且发现新反常**
| 段 | n_max=7 | n_max=3 | 来源 |
|---|---|---|---|
| (b1) **注入前向**（1 token） | **1.73 ms/call** | 1.41 | `inject timing:`（`:1417-1425`，此前未读） |
| (b2) **噪声块前向** | **13.87-13.90** | **13.18-13.20** | `spec timing: draft_decode` |
| (c) selector | **2.91** | 2.01 | `spec timing: selector`；内部 `fetch 0.07 + topk 0.76 + gate 1.02 = 1.85`（余 1.06 未细分） |
| (d) walk | **0.07** | 0.03 | `spec timing: walk` |
| (a) **隐状态 gather + memcpy** | **仍未知** | 仍未知 | `:1393-1403` 无计时 ⇒ 仍需 E1 |

**已测合计 = 18.58 ms/轮**（不含 (a)）；DFlash2 专项成本比此前估的 18.1 略高。

**⚠️ 新的反常（比原假设更重要）**：
1. **注入前向 1.73 ms vs 块前向 13.87 ms —— 同一个模型、同一批权重，差 8.0 倍**；
   而两者唯一的结构差别是：块前向要为**每个 block token 输出 logits**（`common_batch_add(..., !is_dflash2 || is_dflash2_cpu)` `:1465`），
   注入前向 `logits[i]=false`（`:1415`）。draft 的 lm_head 权重 ≈ 827 MB（152k x 5120 x 1.06 B），**按 token 计费**。
2. **但块前向的成本几乎与 block 大小无关**：n=3 时 13.18 ms，n=7 时 13.87 ms（只差 5%）。
   ⇒ 若真是「每 token 一次 lm_head」，n=3 应约 6 ms。**所以 13.5 ms 是一个与 token 数无关的固定成本。**
3. ⇒ 结论：**块前向比注入前向慢了 8 倍，但不是因为 token 多** —— 是这条前向路径本身（图/内核/同步）多花了约 12 ms。
   这与此前「draft 权重只需 0.87 ms」的算法解释相差 15 倍，**必须再往里拆一层**。

### 1.4c E0b（零代码）——**两个新事实，把 13.87 ms 的性质改了**
1. **`draft ctx: n_eval=1 n_reused=0 t_eval=0.0 ms`（每一行都如此，n=7 与 n=3 都一样）**
   => 这个打印**不能用于归因**：`n_eval` 每轮都是 1（像是每轮被重置），`t_eval` 恒为 0.0 ms。
   与 `[OP]` 探针同类问题 —— **`llama_perf_context(ctx_dft)` 的输出在 DFlash2 路径上不可用**，不得引用。
2. **关键：`st_dec` 与 `inj_us` 都是「`llama_decode` 调用的墙钟」，而 `llama_decode` 是异步提交**
   （提交完就返回；真正的执行在 GPU 上、由后面的 `llama_get_logits_ith`/同步点等待）。
   => 所以 **13.87 ms 是「提交块前向」的主机耗时，不是 GPU 执行时间**；1.73 ms 同理是「提交注入前向」的耗时。
   => **8 倍差发生在提交路径上**，而不是算力上 —— 这与我们独立测到的「主机侧逐节点启动是主导项」完全一致
      （`[META] dev`；draft 图同样要经 meta 后端在 3 张卡上各派发一次）。
