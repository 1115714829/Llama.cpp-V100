...
// T1-C tile recipe (from cmake/sm70_79t.cmake _79t_defs - the source of truth).
#define PREFIX_FULL_ENDPOINT
#define PREFIX_TORCH_STABLE_ROWS
#define PREFIX_PV_FP32_MMA_ACCUMULATE
#define PREFIX_TORCH_PREFIX_FP32_OUTPUT
#define PREFIX_TORCH_BLOCK_N 24576
#define PREFIX_QK_PRETRANSPOSE_INPUTS
#define PREFIX_TRANSPOSED_SCORE_WORKSPACE
#define PREFIX_PV_COMPUTE_SUM
#define PREFIX_PV_UNNORMALIZED
#define PREFIX_PV_DIRECT_FP16_ACCUMULATE
#define PREFIX_QK_CUBLAS_RAW
#define PREFIX_BATCHED_TRI_TAIL
#define PREFIX_BATCHED_TAIL_TILE_TOKENS 320
#define PREFIX_BATCHED_TAIL_QK_ALGO CUBLAS_GEMM_ALGO10_TENSOR_OP
#define PREFIX_BATCHED_TRI_REVERSE_PV_TASKS
#define PREFIX_TAIL_IDLE_SM_OVERLAP
#define PREFIX_TAIL_IDLE_SM_FINE_PV
#define PREFIX_TAIL_FINE_PV_GROUP_TILES 4
#define PREFIX_TAIL_FINE_PV_DIRECT_ACCUMULATE
#define PREFIX_BATCHED_TRI_REPAIR_FIRST_TILE
...
struct TailQKBatchedZSwizzle
  ...
  CUTLASS_DEVICE
  cutlass::gemm::GemmCoord get_tile_offset(int log_tile) {
    auto offset = this->Base::get_tile_offset(log_tile);
    ...
    if (t1c_ref_calls++ < ref_max) {
      ...
      for (int r = 0; r < 11; ++r) {
        ...
        fprintf(stderr,
                ...
                got[2]);
        // Score/state bisect: engine scores are half at [col*rows + row].
        {
          ...
          for (int c = 0; c < 5; ++c) {
            ...
            unsigned short hs = 0;
            cudaMemcpy(&hs,
                       (const unsigned short *) ws.scores +
                           (size_t) col * rows + row,
                       sizeof(unsigned short), cudaMemcpyDeviceToHost);
            fprintf(stderr,
