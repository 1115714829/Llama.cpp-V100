...
$CXXBIN $DEFS $INC -O3 -DNDEBUG -pthread -c /root/m5-src/test-backend-ops.cpp -o $OBJ/test-backend-ops.cpp.o > /root/m5-log/testbin.cc.log 2>&1 || { echo TESTBIN_CC_FAIL; tail -20 /root/m5-log/testbin.cc.log; exit 1; }
$CXXBIN -O3 -DNDEBUG $OBJ/test-backend-ops.cpp.o -o $LIB/test-backend-ops -Wl,-rpath,$LIB $LIB/libllama-common.so.0.4.1 $LIB/libllama.so.0.4.1 $LIB/libggml.so.0.24.0 $LIB/libggml-cpu.so.0.24.0 $LIB/libggml-cuda.so.0.24.0 $LIB/libggml-base.so.0.24.0 $BD/common/libllama-common-base.a -lpthread > /root/m5-log/testbin.ld.log 2>&1 || { echo TESTBIN_LD_FAIL; tail -20 /root/m5-log/testbin.ld.log; exit 1; }
echo TESTBIN_BUILT
LD_LIBRARY_PATH=$LIB $LIB/test-backend-ops perf -o FLASH_ATTN_EXT -b CUDA0 -p "nr23=\[6,1\],kv=8192,nb=8" 2>&1 | grep -a -e TFLOPS -e error | head -3
echo TESTBIN_OK
