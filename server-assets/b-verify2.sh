#!/bin/bash
# Post-build real-machine verification battery (v2: no parentheses in echo, no remote sed)
set -u
M=/root/llm/models/Qwen3.8-27B-GGUF/Qwen3.8-27B-Q8_0.gguf
L=/root/libdir-instr
export GGML_CUDA_P2P=1
echo BATTERY_START; date
echo STEP1 regression instrumented-lib env-OFF; date
CARDS=0,1,2 SPLIT=tensor TAG=instr-reg L=$L P2P=1 NODROP=1 PORT=8140 bash /root/p60-ab-harness.sh
echo STEP2 instrumented-timing sync-and-splits; date
...
echo STEP3 per-op plus allreduce counters graphs-OFF; date
...
echo STEP4 M1 prefill TP3-vs-TP4 f16-KV; date
...
echo STEP5 M2 ub-sweep on TP4; date
