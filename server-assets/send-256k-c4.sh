...
echo "=== launch 256k TTFT test (orig, port 8082) in background ==="
nohup bash -c "curl -s -m 3600 http://127.0.0.1:8082/completion -H 'Content-Type: application/json' -H 'Authorization: Bearer $KEY' -d @/tmp/req256k.json -o /tmp/resp256k.json -w 'http=%{http_code} total=%{time_total}s TTFT=%{time_starttransfer}s\n'" > /tmp/bench-256k-orig.log 2>&1 &
