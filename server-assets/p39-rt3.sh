#!/bin/bash
# Phase 0.2c: rebuild after moving the report onto the perf_get_data() channel, then re-test.
set -uo pipefail
...
sed -i 's/\r$//' "$SRC/src/llama-context.cpp" "$SRC/src/llama-context.h"
nohup bash -c "cd $SRC && source /opt/rh/gcc-toolset-12/enable && cmake --build build --config Release -j82 --target llama-server" > /tmp/build-rt3.log 2>&1 &
n=0
while pgrep -f "[c]make --build" >/dev/null 2>&1; do
  sleep 10; n=$((n+10)); [ "$n" -gt 700 ] && { echo "build timeout"; break; }
done
echo -n "build errors: "; grep -icE "error:|FAILED" /tmp/build-rt3.log
tail -2 /tmp/build-rt3.log
...
echo ""
echo "=== [3] short run with LLAMA_ROUND_TIMING=1 ==="
...
echo ""
echo "=== one-time env self-report (from process_ubatch) ==="
grep -a "LLAMA_ROUND_TIMING = " /tmp/rt3-server.log | head -3
echo ""
echo "=== round timing report (from perf_get_data) ==="
grep -a "round timing" /tmp/rt3-server.log | head -8
echo ""
echo "=== sanity: graphs reused + eval time lines present? ==="
grep -a -c "graphs reused" /tmp/rt3-server.log
grep -a "eval time =" /tmp/rt3-server.log | tail -1
echo RT3_DONE
