#!/bin/bash
# p60-ab-harness.sh -- Goal harness: fixed-workload single-stream decode throughput on AC922.
#
# Fixed (comparability) part: model, ctx, sampling, prompt set, seed, n_predict.
# Parametric (agent's decision, must be recorded): card list, split mode, spec config, libdir.
#
# Usage:
#   CARDS="0,1,2" SPLIT=layer TAG=base bash p60-ab-harness.sh
# Env knobs (all optional):
#   CARDS (default 0,1,2) SPLIT (tensor|layer|none, default tensor) TAG (default base)
#   NPRED (default 512)   PORT (default 8120)  L (libdir snapshot)  SPEC (spec args)
set -uo pipefail
...
# --- hygiene: this box maps V100 HBM2 into system RAM, so host page cache eats VRAM.
#     NODROP=1 skips the cache drop for fast diagnostic iteration (load ~15 s instead of ~270 s);
#     any measurement that is quoted as a result must be taken with the drop enabled.
if [ "${NODROP:-0}" = "1" ]; then
...
# --- fixed prompt set (3 prompts, same text for every arm forever) ---
...
# optional experiment switches: P2P=1 enables cudaDeviceEnablePeerAccess across devices
# (ggml-cuda.cu:391 reads GGML_CUDA_P2P); set ARENV="A=1,B=2" for GGML_CUDA_AR_* knobs
if [ "${P2P:-0}" = "1" ]; then
...
# optional draft placement: DEVD="CUDA0" pins the draft model to a single device (avoids its
# all-reduce entirely, which the TP3 measurements show costs ~10 ms/round on a 1.14 GB model)
...
# NOGRAPH=1 disables CUDA graph capture (A/B for whether graphs are actually replayed)
if [ "${NOGRAPH:-0}" = "1" ]; then
  export GGML_CUDA_DISABLE_GRAPHS=1
  echo "extra env: GGML_CUDA_DISABLE_GRAPHS=1" | tee -a "$OUT"
fi

env CUDA_VISIBLE_DEVICES="$CARDS" LD_LIBRARY_PATH="$L" LLAMA_ROUND_TIMING=1 LLAMA_SPEC_TIMING=1 \
  nohup "$L/llama-server" --model "$M" $SPEC $DRAFT_ARGS --split-mode "$SPLIT" --tensor-split "$TS" \
    --ctx-size 8192 -ngl 999 \
    --flash-attn on --cache-type-k $CTKV --cache-type-v $CTKV --parallel 1 \
...
if [ "$ok" != "1" ]; then
  ...
  grep -aE "GGML_ASSERT|error|failed to|abort|not supported|no memory" "$SLOG" | tail -8 | tee -a "$OUT"
  kill "$pid" 2>/dev/null; sleep 3; kill -9 "$pid" 2>/dev/null
  echo "P60_DONE TAG=$TAG STATUS=LAUNCH_FAILED"
  exit 1
fi

# --- throughput: 3 fixed prompts, official thinking-mode sampling, seed 42 ---
VALS=""
i=0
...
# --- correctness probe: greedy (temp 0) fixed prompt, for token/text identity across arms ---
