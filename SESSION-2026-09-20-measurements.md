...
2. **真正的靶心是 `dev` = 13.58 ms/call（72%）**。`sub/call=49.3` => 每 call 约 49 子图 x 3 设备 ≈ 148 次设备调用
   ...
   **capture / replay / direct 比例 = 5.3% / 61.2% / 33.5%**。
   ...
   - **两臂确认（Round 115）**：dirA `direct = 17345/124160 = 14.0%`；dirB（跑到 92672 次调用时）`capture=1662 replay=79605 direct=11405` => **direct = 12.3%**。
     **两臂完整结果（Round 116，`/tmp/direct-ab.txt`，均可复现）**：
     | 量 | dirA | dirB | 离散 |
     |---|---:|---:|---:|
     | `[GRAPH] calls` | 124160 | 124160 | **0** |
     | capture | 2838 | 2838 | **0** |
     | replay | 103977 | 103977 | **0** |
     | **direct** | **17345** | **17345** | **0** |
     | tg / AL | 99.97 / 5.55 | 99.18 / 5.55 | 0.8% / 0 |
     | `[META] total` | 10.136 | 10.563 | 4% |
     | `[META] dev` | 6.808 | 7.209 | 6% |
     | `[META] ar` | 2.202 | 2.199 | 0.1% |
     ~ **capture/replay/direct 三项计数在两臂间逐位相同** => 该现象**完全确定性**，是可靠的优化目标。
     稳态 direct = **17345/124160 = 13.97%**。（短跑 NPRED=64 的 33.5% 是启动段污染，已作废。）
     早期采样也印证：dirB `calls=256` 时 direct=100%、`calls=2560` 时 81% => 启动段权重极大，跑得越久越被稀释。
     => 这 14% 在真实长会话里是**持续存在**的代价，不是一次性开销。
   ...
   - ✅ **探针结果（Round 131，`/tmp/p60-dirprobe-server.log`，TAG=dirprobe，STATUS=OK，MEDIAN_TG=97.51）**：
     累计 `changes=323584`，按字段命中次数排序（括号内为占 changes 的比例）：
     ```
     src_nb=262698 (81.2%)   src_ne=253471 (78.3%)   nb=251595 (77.8%)   ne=231767 (71.6%)
     src_data=83839 (25.9%)  data=83242 (25.7%)      src_ptr=82350 (25.4%)
     view=52945 (16.4%)      op=48786                flags=48786   buffer=48786   name=48786
     op_params=41959         type=1122               extra=0       other=0
     src_data_j=[82174, 23193, 1014, 621, 432, 432, 0,0,0,0]
     ```
     **结论：属性抖动的主体是「形状」（`ne`/`nb`，节点自身与其 src 的），不是指针（约 3-4 倍于后者）。**
     `op/flags/buffer/name` 都等于 48786，正是「该次比较是新条目、全字段都算变」的次数（基线）；
     在此之上真正增长的是 `src_nb`(+21.4 万)、`ne`(+18.3 万)，而 `data`(+3.4 万)、`view`(+0.4 万) 小一个量级。
   - **最频繁的个体节点**：`attn_norm-0`（69 次）、`cache_r_l0 (reshaped)` 及其多层 view（各 30 次），
     另有 `conv_input-*`、`k_conv`、`Qcur_full`、`z-*`、`beta-*`、`cache_s_l*` —— **全部是 GDN / delta-net 的递归状态视图**。
     => **形状抖动集中在递归状态（A2 那块），不是 KV cache 视图。**
   - **这直接改写了修法选择**（本规格 §4 的三选一）：
     首位是 `ne`/`nb` => **必须走形状稳定化（P-C 形状分桶），而不是「稳定分配」**；
     且抖动源是递归状态视图 => **P-C 与 A2 合流**，二者应一起做。
   - 另一个副产品：`nodes_updated_without_memcmp=388083` —— 因为 `ggml-cuda.cu:2776` 的条件是 `if (res || memcmp(...))`，
     **一旦某个节点判为变化，其后所有节点都被直接标记为变化、不再 memcmp**，所以「首个不同项」才是关键，字段计数会被放大。
     下一版若要更精确，应在**每次调用只统计首个不同项**。
