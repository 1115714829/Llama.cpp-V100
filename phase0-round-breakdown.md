...
⇒ **正确的比值是：整轮 vs 自身 M=1 轮 = 92.6 / 32.35 ≈ 2.9×**（两轮都只读一遍权重）。
⚠️ **2026-09-20 DSH 审计更正**：原文写「纯带宽理想下 M=8 每行应 = 32.35/8 = 4.0 ms」是**推导错误** ——
32.35 ms 是 M=1 的**整轮**（它自己已是 9.7 GB/卡 硬下限 10.8 ms 的 ~3×），不能除以 8 当作每行的带宽下限。
正确的硬下限见 `goal-phase1-findings.md` §4：**10.8 ms/轮** ⇒ 当时 M=8 目标前向 29.7 ms = 2.8×、整轮 73 ms = 6.8×。
⇒ 结论方向不变（**存在大量非带宽成本**），但**不能把 2.9× 解读成 HMMA 能拿回 2.9×**：
   该 GEMM 算子自身已在其 n=1 屋顶的 73%，天花板见 `goal-phase1-findings.md` §18（≤1.36×）。
...
| ~~Phase 2.8 M≈8 GEMM 走 Tensor Core~~ | ~~M=8 每行 11.6 ms vs 带宽下限 4.0 ms ⇒ 有 ~2.9× 空间~~ | ⚠️ **已撤回**（推导错误 + §18 实测：该算子 ≤1.36×）。现行优先级见 `AUDIT-2026-09-20-dsh.md` |
