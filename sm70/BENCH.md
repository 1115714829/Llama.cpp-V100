...
| 层 | 目的 | 硬件 | 模型 | 耗时 | 工具 |
|---|---|---|---|---|---|
| **L1 算子级** | 内核正确性与速度（日常迭代） | 单卡 GPU 2 | 无（真实形状文件） | 秒级 | `tools/op-perf.py` |
| **L2 贪心门** | 模型级正确性 | GPU 2（IQ1_S）/ 卡 0,1,3,4（Q8_0） | 见下 | 分钟级 | `tools/gate.py` |
| **L3 256K 标准测试** | 基线、验收、里程碑 A/B | 卡 0,1,3,4 | Q8_0 + F16 草稿 / vLLM 原样 | ≈15 min/臂 | `tools/bench-256k.sh` |
