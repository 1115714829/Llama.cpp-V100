...
- tensor 模式（修 selector 后）：每轮 ≈ 73 ms = target **29.7** + draft **15.1** + selector **4.4** + walk 0.09 + host **2.4**
  ⇒ 合计 51.6 ms，尚有 ~21 ms 未归因。
  ⚠️ **2026-09-20 DSH 审计更正**：这是 **NCCL 之前**的账本；当前最好构建（NCCL+P2P）是 **58.9 ms/轮**：
  target 整步 **32.3–34.3**（含 21.8 ms 异步提交窗口 + 2.3 主机图工作 + ~9 尾部）+ draft **13.8–14.4** + selector **4.3** + walk 0.1 ⇒ **未归因只剩 ~6–8 ms**。
  另：`enqueue_us` 那句「仅 M>1 同步所以可信」**不成立**（见 §5 更正）。
...
**字节/卡的口径（⚠️ 2026-09-20 DSH 审计已更正：原表述错误）**

> 🛑 原文（保留作历史）：1cat 用 4-bit（~13.5 GB/4 卡 ≈ 3.4 GB/卡/轮），我们 Q8_0 29 GB/3 卡 = 9.7 GB/卡/轮 ⇒ 他们少读 ~2.9×；所以 2.9× 来自字节数、只有 1.3× 来自效率。
> **错在哪**：把我们实测的那个 1cat 服务当成了 4-bit（NVFP4）。
> **实测事实**（`/root/llm/systemd/vllm-1cat.service` 跑的就是它）：模型目录 `Qwen3.8-27B-FP8` = **30,866,866,928 B**（66 个 safetensors）+ draft `Qwen3.8-27B-DFlash2` = **3,848,817,896 B** ⇒ 合计 **34.72 GB / 4 卡 = 8.68 GB/卡/轮**。
> **我们**：Q8_0 29,047,086,048 B + draft 1,143,006,816 B = 30.19 GB / 3 卡 = **10.06 GB/卡/轮**。
> ⇒ **只差 1.16×，不是 2.9×**。差距几乎全在**效率**：每轮 58.9 ms vs 17.463 ms = **3.37×**（每卡有效带宽 ~171 GB/s 对 ~497 GB/s）。
> 另：**换成 4 卡后我们是 30.19/4 = 7.55 GB/卡，反而优于 1cat 的 8.68** —— 卡数 1–6 自由（用户 2026-09-20 明确），字节不再是任何借口。

（带宽下限换算仍可用：9.7 GB/卡 ÷ ~900 GB/s ≈ **10.8 ms/轮**；我们整轮 58.9 ms ≈ 5.5× 下限。）
...
| 卡数（TP2/TP3/TP4/TP6） | 当时测得 TP3 最优。⚠️ **已降级为历史实测**：用户 2026-09-20 明确**卡数 1–6 自由、跨岛不是瓶颈**，需在 Phase B4 重扫（不得据此限死 3 卡） |
...
| ~~唯一剩余量级杠杆~~ | ⚠️ **本行与本文档 §18 自相矛盾，以 §18 为准**：该算子已在其自身屋顶的 73%，理论 ≤1.36×，**不是**唯一杠杆。 |
| **审计后的真实优先级** | ① 长上下文 attention（本模型 **head_dim=256 / 24 Q 头 / 4 KV 头 / 16 层全注意力**，此前几何写错）② DFlash2 编排（draft graph、selector 上 GPU）③ prefill/TTFT（256K 仅 370 t/s，1cat 公开 32K/64K 为 3,567–4,069 t/s）④ 多设备 allreduce ⑤ HMMA GEMM。详见 `AUDIT-2026-09-20-dsh.md` 与 `1CAT-PORT-BACKLOG.md` |
...
- ⇒ 与 1cat 的 ~4× 差距，**不能归因于「M=8 GEMM 没用 Tensor Core」**；结构性差异更可能来自：**融合管线 / 每轮编排开销**。
  ⚠️ **2026-09-20 DSH 审计更正本行的字节与接受率部分**：
  - 字节：1cat 实测服务是 **FP8**（34.72 GB/4 卡 = **8.68 GB/卡**），我们 **10.06 GB/卡** ⇒ **只差 1.16×**（不是原写的 2.9×；4 卡时我们 7.55 GB/卡反超）；
  - 接受率：1cat v1.5.0 公开 GSM8K 契约 **AL request-mean 5.3888 / pooled 4.5644**（官方 5.46），我们 **4.11–6.08** ⇒ **接受长度不是差距来源**。
